...
Примечания важно иметь доступ к моделям с OpenRouter то что в названиях моделей написано free например "tngtech/deepseek-r1t2-chimera:free"  не дает полного доступа модели есть ограничения токенов причем очень мало их на модель. Ну так вот нужно будет  API OpenRouter, API от Huggingface.co на этом все приложение в минималистичном варианте не стал запариватся с визуалом тк делал для себя. Если вам не жалко несколько рублей не могли бы вы поддержать меня.
