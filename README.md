...
Примечания важно иметь доступ к моделям с OpenRouter если вы тут вы наверно Програмист. Ну так вот нужно будет  API OpenRouter, Api от Huggingface.co на этом все приложение в минималистичном варианте не стал запариватся с визуалом тк делал для себя. Если вам не жалко несколько рублей не могли бы вы поддержать меня.
