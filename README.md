...
Поддержат копеечкой https://www.tbank.ru/cf/9PWDCSkj2FL 

Примечания важно иметь доступ к моделям с OpenRouter то что в названиях моделей написано free например "tngtech/deepseek-r1t2-chimera:free"  не дает полного доступа модели есть ограничения токенов причем очень мало их на модель. Ну так вот нужно будет  API OpenRouter, API от Huggingface.co на этом все приложение в минималистичном варианте не стал запариватся с визуалом тк делал для себя. Если вам не жалко несколько рублей не могли бы вы поддержать меня.ЕЩЕ ВАЖНОЕ ПРИМЕЧАНИЕ ЭТОТ ПРОЕКТ ОБСОЛЮТНО БЕЗОПАСЕН МОЖЕТЕ ЗАЛИТЬ НА ВИРУС ТОТАЛ ПОКАЖЕТ ТОЛЬКО УЯЗВИМОСТЬ ТОКЕНОВ ИЛИ API ТК ВСЕ НАВЕРНО НЕ УДАЛИЛ ТК СЛИШКОМ МНОГО ПОЛУЧИЛОСЬ ВАЙЛОВ И Я ВПИСЫВАЛ ИХ ВРУЧНУЮ НЕ ЧЕРЕЗ .env

Скачать: [скачать](https://disk.yandex.ru/d/a9CvNtZkmeJo-Q)
